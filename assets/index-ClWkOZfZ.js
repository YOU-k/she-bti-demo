(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))l(e);new MutationObserver(e=>{for(const o of e)if(o.type==="childList")for(const s of o.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&l(s)}).observe(document,{childList:!0,subtree:!0});function n(e){const o={};return e.integrity&&(o.integrity=e.integrity),e.referrerPolicy&&(o.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?o.credentials="include":e.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function l(e){if(e.ep)return;e.ep=!0;const o=n(e);fetch(e.href,o)}})();function N(t,a){const n={};for(const l of a)t[l.id]!=null&&(n[l.dim]=(n[l.dim]||0)+t[l.id]);return n}function D(t,a){const n={};for(const[l,e]of Object.entries(t))e<=a.L[1]?n[l]="L":e>=a.H[0]?n[l]="H":n[l]="M";return n}const I={L:1,M:2,H:3};function F(t){return t.replace(/-/g,"").split("")}function O(t,a,n){const l=F(n);let e=0,o=0;for(let r=0;r<a.length;r++){const h=I[t[a[r]]]||2,c=I[l[r]]||2,b=Math.abs(h-c);e+=b,b===0&&o++}const s=a.length*2,i=Math.max(0,Math.round((1-e/s)*100));return{distance:e,exact:o,similarity:i}}function Q(t,a,n,l){const e=n.map(i=>({...i,...O(t,a,i.pattern)}));e.sort((i,r)=>i.distance-r.distance||r.exact-i.exact||r.similarity-i.similarity);const o=e[0],s=l.find(i=>i.code==="PATHFINDER");return o.similarity<55&&s?{primary:{...s,similarity:o.similarity,exact:o.exact},secondary:o,rankings:e,mode:"fallback"}:{primary:o,secondary:e[1]||null,rankings:e,mode:"normal"}}function j(t){return[...t.main]}function z(t,a,n,l){const{onAnswer:e,onBack:o,currentAnswer:s}=l,i=document.createElement("div");i.className="quiz-question";const r=a>0;i.innerHTML=`
    <div class="progress">
    ...
    <p class="question-text">${K(t.text)}</p>
    <div class="option-list"></div>
  `,r&&i.querySelector(".back-btn").addEventListener("click",o);const h=i.querySelector(".option-list");return t.options.forEach(c=>{const b=document.createElement("button");b.className="option-btn"+(s===c.value?" option-btn--selected":""),b.type="button",b.textContent=c.label,b.addEventListener("click",()=>e(t.id,c.value)),h.appendChild(b)}),i}function K(t){return String(t).replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a])}const V={L:1,M:2,H:3},C={ring:["rgba(230, 108, 122, 0.10)","rgba(230, 108, 122, 0.06)","rgba(230, 108, 122, 0.03)"],grid:"rgba(230, 108, 122, 0.18)",axis:"rgba(230, 108, 122, 0.14)",label:"#a25763",polyFill:"rgba(230, 108, 122, 0.28)",polyStroke:"rgba(210, 82, 96, 0.85)",dot:"#d25260"};function X(t,a,n,l){var q;const e=t.getContext("2d"),o=window.devicePixelRatio||1,s=320;t.width=s*o,t.height=s*o,t.style.width=s+"px",t.style.height=s+"px",e.scale(o,o);const i=s/2,r=s/2,h=s/2-44,c=n.length,b=Math.PI*2/c,H=-Math.PI/2;e.clearRect(0,0,s,s);for(let d=3;d>=1;d--){const g=d/3*h;e.beginPath(),e.arc(i,r,g,0,Math.PI*2),e.fillStyle=C.ring[3-d],e.fill(),e.strokeStyle=C.grid,e.lineWidth=.5,e.stroke()}e.font='10px system-ui, -apple-system, "PingFang SC", sans-serif',e.textAlign="center",e.textBaseline="middle";for(let d=0;d<c;d++){const g=H+d*b,y=i+Math.cos(g)*h,w=r+Math.sin(g)*h;e.beginPath(),e.moveTo(i,r),e.lineTo(y,w),e.strokeStyle=C.axis,e.lineWidth=.5,e.stroke();const f=h+22,R=i+Math.cos(g)*f,Y=r+Math.sin(g)*f,_=n[d],B=((q=l[_])==null?void 0:q.name)||_;e.fillStyle=C.label,e.fillText(B,R,Y)}const M=n.map(d=>V[a[d]]||2);e.beginPath();for(let d=0;d<c;d++){const g=H+d*b,y=M[d]/3*h,w=i+Math.cos(g)*y,f=r+Math.sin(g)*y;d===0?e.moveTo(w,f):e.lineTo(w,f)}e.closePath(),e.fillStyle=C.polyFill,e.fill(),e.strokeStyle=C.polyStroke,e.lineWidth=2,e.stroke();for(let d=0;d<c;d++){const g=H+d*b,y=M[d]/3*h,w=i+Math.cos(g)*y,f=r+Math.sin(g)*y;e.beginPath(),e.arc(w,f,3,0,Math.PI*2),e.fillStyle=C.dot,e.fill()}}const ee="https://cdn.jsdelivr.net/npm/html2canvas@1.4.1/dist/html2canvas.min.js";let Z=null;function te(){return window.html2canvas?Promise.resolve(window.html2canvas):Z||(Z=new Promise((t,a)=>{const n=document.createElement("script");n.src=ee,n.onload=()=>t(window.html2canvas),n.onerror=()=>a(new Error("html2canvas failed to load")),document.head.appendChild(n)}),Z)}async function ae(t,a="lbti-result.png"){const e=(await(await te())(t,{backgroundColor:"#fdf5f3",scale:2,useCORS:!0,logging:!1})).toDataURL("image/png");ne(e,a)}function ne(t,a){const n=/MicroMessenger/i.test(navigator.userAgent),l=/Mobile|Android|iPhone|iPad/i.test(navigator.userAgent),e=n?"👇 长按图片，选择「保存图片」到相册":l?"👇 长按图片保存到相册（或用下方按钮下载）":"👇 右键图片保存，或用下方按钮下载",o=document.createElement("div");o.className="img-modal-backdrop",o.innerHTML=`
    <div class="img-modal">
      ...
      ${n?"":`<a class="btn btn-primary img-modal-dl" download="${le(a)}" href="${t}">下载图片</a>`}
    </div>
  `,document.body.appendChild(o);const s=()=>o.remove();o.querySelector(".img-modal-close").addEventListener("click",s),o.addEventListener("click",r=>{r.target===o&&s()});const i=r=>{r.key==="Escape"&&(s(),document.removeEventListener("keydown",i))};document.addEventListener("keydown",i)}function le(t){return String(t).replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a])}function se(t,a,n){const{primary:l,secondary:e}=t,o=e&&t.mode==="normal"?`
次匹配：${e.cn}（${e.similarity}%）`:"",s=ie(n,l.code,l.similarity);return[`【${a}】`,`我是「${l.cn}」${l.slogan?"—— "+l.slogan:""}`,`匹配度：${l.similarity}%${o}`,"",`📎 看我的结果卡 + 测你的：${s}`].join(`
`)}function ie(t,a,n){const l=new URL(t);return l.searchParams.set("r",a),l.searchParams.set("s",String(n)),l.toString()}function oe(){const t=new URLSearchParams(window.location.search),a=t.get("r");if(!a)return null;const n=parseInt(t.get("s")||"0",10);return{code:a,similarity:Number.isFinite(n)?n:0}}async function re(t){if(navigator.clipboard&&window.isSecureContext)return await navigator.clipboard.writeText(t),!0;const a=document.createElement("textarea");a.value=t,a.style.position="fixed",a.style.opacity="0",document.body.appendChild(a),a.select();const n=document.execCommand("copy");return document.body.removeChild(a),n}function ce({result:t,dimensions:a,config:n,standardTypes:l,userLevels:e,onRestart:o}){const{primary:s,secondary:i,rankings:r,mode:h}=t,c=document.createElement("div");c.className="result-page";const H=(h==="normal"?s:r[0]).pattern.replace(/-/g,"").split(""),M=de(e||{},H,a,l),q=h==="fallback"?"（拓荒者 · 尚未命名型）":"",d=h==="normal"&&i&&s.similarity===i.similarity,g=d?`<div class="result-hybrid-badge">你在两个型之间 · ${m(s.cn)} × ${m(i.cn)}</div>`:"",y=Object.fromEntries(l.map(p=>[p.code,p])),w=(s.match_high||[]).map(p=>{var v;return(v=y[p])==null?void 0:v.cn}).filter(Boolean),f=(s.match_low||[]).map(p=>{var v;return(v=y[p])==null?void 0:v.cn}).filter(Boolean),R=r.slice(0,5);c.innerHTML=`
    <header class="result-header">
      <div class="result-mode">${q}</div>
      ${g}
      <h1 class="result-name">${m(s.cn)}</h1>
      <p class="result-intro">${m(s.intro)}</p>
      <div class="result-similarity">
        与原型相似度 <b>${s.similarity}%</b>${i&&h==="normal"?` · 次匹配 <b>${m(i.cn)}</b> ${i.similarity}%`:""}
      </div>
      ${d?`
        <p class="result-hybrid-note">你的答题分布让两个型跟你距离很近 —— 你不是完美的 ${m(s.cn)}，也不是完美的 ${m(i.cn)}，你是这两个型的<b>混合体</b>。下面的描述以主匹配为主，你可以顺手看看次匹配那个型你身上有几分。</p>
      `:""}
    </header>

    <section class="result-desc">
      <p>${P(s.desc)}</p>
      ${s.slogan?`<blockquote class="result-slogan">「${m(s.slogan)}」</blockquote>`:""}
    </section>

    ${s.warning?`
      <section class="result-warning">
        <div class="warning-label">📢 给 ta 的一句话警告</div>
        <p class="warning-body">${m(s.warning)}</p>
      </section>
    `:""}

    ${i&&h==="normal"&&i.similarity>=55?`
      <section class="result-secondary">
        ...
        <details class="secondary-more">
          ...
          <p class="secondary-desc">${P(i.desc)}</p>
    ...
    ${M.length>0?`
      <section class="result-why">
        <h3>为什么是这个型？</h3>
        <p class="why-lead">决定你身份的 ${M.length} 个关键维度：</p>
        <ul class="why-list">
          ${M.map(p=>`
            <li>
    ...
    ${h==="normal"&&(w.length||f.length)?`
      <section class="result-match">
        <h3>与你最合拍的 & 最相克的</h3>
        <div class="match-grid">
          <div class="match-col">
            <div class="match-label match-label-high">同频（最能理解你）</div>
            <div class="match-tags">${w.map(p=>`<span class="tag tag-high">${m(p)}</span>`).join("")}</div>
          </div>
          <div class="match-col">
            <div class="match-label match-label-low">相克（走的路不同）</div>
            <div class="match-tags">${f.map(p=>`<span class="tag tag-low">${m(p)}</span>`).join("")}</div>
          </div>
        </div>
      </section>
    `:""}
    ...
    <footer class="result-footer">
      <p>${m(h==="normal"?n.display.funNote:n.display.funNoteSpecial)}</p>
    </footer>
  `;const Y=c.querySelector("#radar"),_=e&&Object.keys(e).length>0?e:ue(r[0].pattern,a.order);X(Y,_,a.order,a.dims);const B=window.location.origin+window.location.pathname;c.querySelector("#copy-btn").addEventListener("click",async()=>{const p=se(t,n.display.title,B),v=await re(p),$=c.querySelector("#copy-btn");$.textContent=v?"已复制！粘贴到朋友圈就行":"复制失败，长按选中",setTimeout(()=>$.textContent="复制结果给朋友炫耀",3e3)});const L=c.querySelector("#save-img-btn");return L.addEventListener("click",async()=>{const p=L.textContent;L.textContent="生成中...",L.disabled=!0;const v=c.querySelector(".result-actions"),$=c.querySelector(".result-footer");v.style.visibility="hidden",$&&($.style.visibility="hidden");try{await ae(c,`SheBTI-${s.cn.replace(/[/\s]/g,"")}.png`),L.textContent="✓ 图片已生成"}catch(W){L.textContent="生成失败",console.error(W)}finally{v.style.visibility="",$&&($.style.visibility="")}setTimeout(()=>{L.textContent=p,L.disabled=!1},3e3)}),c.querySelector("#restart-btn").addEventListener("click",o),c}function de(t,a,n,l){const e=n.order,o=[];for(let s=0;s<e.length;s++){const i=e[s],r=t[i],h=a[s];if(!r||r!==h)continue;let c=0;for(const M of l)M.pattern.replace(/-/g,"")[s]===r&&c++;const b=n.dims[i];if(!b)continue;const H=r==="H"?b.hi:r==="L"?b.lo:"介于两端之间";o.push({dim:i,level:r,name:b.name,hint:H,shared:c})}return o.sort((s,i)=>{if(s.shared!==i.shared)return s.shared-i.shared;const r=s.level==="M"?1:0,h=i.level==="M"?1:0;return r-h}),o.slice(0,3)}function P(t){return m(t).replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>")}function ue(t,a){const n=t.replace(/-/g,"").split(""),l={};for(let e=0;e<a.length;e++)l[a[e]]=n[e];return l}function m(t){return String(t).replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a])}function he({shared:t,allTypes:a,onStartMine:n}){const l=a.find(o=>o.code===t.code),e=document.createElement("div");return e.className="shared-page",l?(e.innerHTML=`
    <div class="shared-inner">
      <div class="shared-badge">你朋友的历史坐标是</div>
      <h1 class="shared-name">${A(l.cn)}</h1>
      <p class="shared-intro">${A(l.intro)}</p>
      ${t.similarity?`<div class="shared-sim">匹配度 <b>${t.similarity}%</b></div>`:""}

      ${l.slogan?`
        <blockquote class="shared-slogan">「${A(l.slogan)}」</blockquote>
      `:""}

      ${l.warning?`
        <div class="shared-warning">
          <div class="shared-warning-label">📢 跟她相处前的一句提醒</div>
          <p class="shared-warning-body">${A(l.warning)}</p>
        </div>
      `:""}
  ...
  `,t.querySelector("#start-btn").addEventListener("click",()=>{u.view="quiz",u.currentIdx=0,u.answers={},x()}),k.appendChild(t)}function Le(){const t=u.order[u.currentIdx],a=z(t,u.currentIdx,u.order.length,{onAnswer:(n,l)=>{u.answers[n]=l,u.currentIdx<u.order.length-1?(u.currentIdx++,x()):(u.view="result",x())},onBack:()=>{u.currentIdx>0&&(u.currentIdx--,x())},currentAnswer:u.answers[t.id]});k.appendChild(a)}function He(){const t=N(u.answers,J.main),a=D(t,S.scoring.levelThresholds),n=Q(a,G.order,E.standard,E.special),l=ce({result:n,dimensions:G,config:S,standardTypes:E.standard,userLevels:a,onRestart:()=>{u.view="intro",x()}});k.appendChild(l),window.scrollTo(0,0)}function T(t){return String(t).replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a])}x();
